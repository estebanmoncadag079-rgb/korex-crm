...
var SALON = {
  ...
  queVende: "Ofrecemos servicio de arreglo de cejas, pesta\xF1as y labios, y tambi\xE9n u\xF1as.",
  ubicacion: "Jamund\xED, Valle del Cauca",
  ...
  tono: "Trato c\xE1lido y especial con cada clienta; usa emojis de cejas y pesta\xF1as en la conversaci\xF3n. Puedes ser un poco gracioso sin perder el hilo y siempre con mucho respeto.",
  reglasPropias: ["Los festivos no trabajamos."],
  preguntasFrecuentes: [],
  escalarSiempre: [],
  nuncaPrometer: ["Devoluci\xF3n del dinero por separaci\xF3n de citas."]
};
CLIENTES.push(
  { organizationId: "org_lo5gdlt6k43z9fg1ling", ficha: CHURRA },
  { organizationId: "org_novxv78s08h12arzatr2", ficha: SALON }
);
var aplicar = process.argv.includes("--aplicar");
var generados = CLIENTES.map((c) => ({ ...c, perfil: generarPerfil(c.ficha) }));
for (const g of generados) {
  console.log(
    `[fichas] ${g.ficha.nombre}: prompt ${g.perfil.instructions.length} caracteres \xB7 escalado ${g.perfil.escalationRules.length}`
  );
...
if (!aplicar) {
  console.log("[fichas] NO se escribi\xF3 nada (falta --aplicar)");
  process.exit(0);
}
var sql2 = postgres(process.env.DATABASE_URL, { max: 1, onnotice: () => {
} });
var db = drizzle(sql2, { schema: schema_exports });
await sql2`CREATE TABLE IF NOT EXISTS agent_profile_bk_fichas_13ago AS
...
for (const g of generados) {
  ...
  }).where(eq(agentProfile.organizationId, g.organizationId));
