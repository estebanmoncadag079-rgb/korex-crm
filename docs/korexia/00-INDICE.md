...
| [137-REQUISITOS-COMO-PASO-DEL-ALTA.md](137-REQUISITOS-COMO-PASO-DEL-ALTA.md) | ✅ **"Qué datos pedir antes de confirmar" ahora también es el paso 8 del alta**, no solo una tarjeta de "Agente de IA" que se visita después. Cuidado con `cierre.requisitos` y `pagoAntesDeLaCita`: viven en la misma sección de la ficha (`flujo`) y `fusionarFicha` fusiona por sección completa — se preserva `pagoAntesDeLaCita` explícitamente para no repetir el incidente del 15-ago. Sin prueba de integración específica ni prueba manual en navegador — pendiente antes de confiar del todo con un cliente de citas |
