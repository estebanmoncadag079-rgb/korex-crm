...
| [146-VERIFICACION-FACTUAL-FORZADA-PAGO.md](146-VERIFICACION-FACTUAL-FORZADA-PAGO.md) | 🔑 **La propia traza del 145 reveló que "¿Puedo pagar por Nequi?" acertaba sin verificar** — mismo molde que el 143 (productos), ahora en pago: `detectarConsultaFactualDeMedioPago` distingue "¿aceptan X?" de preguntas abiertas ("cómo puedo pagar") y fuerza `resolverMetodoDePago` ANTES de la primera llamada al modelo, reutilizando 100% de la infraestructura existente (guardarraíl, flag, traza) sin tocar una línea de código ya validado. Solo Lis se activa (ya tenía ambas condiciones); La Churra y Lashes Valen sin cambios, confirmado con consulta real. 1110 pruebas en verde |
| [145-TRAZA-DIAGNOSTICA-POR-TURNO.md](145-TRAZA-DIAGNOSTICA-POR-TURNO.md) | 🔑 **"El bot respondió algo incorrecto" ya no exige reconstruir el turno a mano cruzando cinco prefijos de log distintos** — una sola línea `[traza]` por turno (mismo patrón que `registrarMetricaDeEstado`, sin tabla nueva) consolida qué se detectó, qué hecho se verificó contra backend, qué guardarraíl actuó, si hubo recuperación de una salida parcial, y la causa exacta del handoff si lo hubo. Nunca vuelca el texto del cliente (huella + `mensajeId`, el texto real sigue en `message`). Hallazgo nuevo de la prueba real: `consultar_medio_pago` tiene el mismo problema que tenía producto antes del 143 — el modelo puede responder sobre un método de pago sin verificar, documentado pero no corregido en esta ronda. 1089 pruebas en verde |
| [144-RECUPERACION-SALIDAS-PARCIALMENTE-VALIDAS.md](144-RECUPERACION-SALIDAS-PARCIALMENTE-VALIDAS.md) | 🔴 **"Hola, buenas noches" escalaba a una persona el 71% de las veces contra Lis** (`handoffReason='error'`): el modelo elegía bien `send_menu` pero omitía `reply` (un campo AUXILIAR, no el corazón de la acción), y el camino de la Fase 2 (`chatJsonConEstado`, que valida `AgentAction` DESPUÉS de que `chatJson` ya dio la llamada por buena) no tenía ninguna red de reintentos para eso. Auditadas las 12 acciones: `send_menu.reply` era el único caso con fallback real ya escrito y nunca alcanzado (`armarMenuDeIntenciones` ya traía "¿Cómo te ayudo hoy?" por defecto) — se quitó la exigencia de Zod. Más un reintento acotado (una sola vez, con el detalle exacto de Zod) para campos que sí son el corazón de su acción. 8/8 repeticiones reales sin handoff. 1075 pruebas en verde |
| [143-VERIFICACION-FACTUAL-FORZADA.md](143-VERIFICACION-FACTUAL-FORZADA.md) | 🔑 **La prueba controlada con Lis reveló que el modelo no siempre decide llamar `consultar_producto`** — con "¿tienen torta de chocolate?" a veces resolvía directo con el catálogo en prosa. Nuevo `detectarConsultaFactualDeProducto` (reglas de texto, sin IA) detecta preguntas factuales concretas ("¿tienen X?", "¿cuánto cuesta X?") y dispara la verificación backend ANTES de la primera llamada al modelo — sin forzar nada en preguntas abiertas o interpretativas ("qué me recomiendas", "para 15 personas"). Flag nuevo `consultas_verificadas_enabled` por organización, encendido SOLO para Lis (confirmado con consulta real: La Churra y las demás en `false`). Diagnóstico aparte (sin tocar código): "Hola, buenas noches" escaló con `handoffReason='error'` en 5/7 intentos — el modelo omite el `reply` obligatorio de `send_menu` de forma intermitente; causa trazable, a diferencia del 127. 1068 pruebas en verde |
| [142-CONSULTAR-PRODUCTO-Y-MEDIO-DE-PAGO.md](142-CONSULTAR-PRODUCTO-Y-MEDIO-DE-PAGO.md) | 🔑 **`consultar_producto`/`consultar_medio_pago`: el mismo molde de `consult_availability` (citas), clonado para pedidos** — el backend calcula el hecho real contra el catálogo/ficha y se lo devuelve al modelo en el mismo turno, en vez de dejar que lo decida leyendo prosa. Nace de la auditoría "el LLM no debe decidir hechos que el backend pueda verificar" sobre el incidente de Lis ("torta de chocolate") y el caso Nequi. Nuevo guardarraíl simétrico (`contradiceProductoEncontrado`/`niegaMetodoDePagoPermitido`): si el modelo consulta y aun así contradice el hecho verificado, se corrige solo. No llega a producción hasta `regenerar:flota`. 1043 pruebas en verde |
| [141-MENU-GUIADO-DE-WHATSAPP.md](141-MENU-GUIADO-DE-WHATSAPP.md) | 🔑 **El cliente TOCA una opción en vez de escribir texto que el modelo tiene que adivinar** — nace de que el bot de Lis no conectó "torta de chocolate" con "Porción Chocolate" del catálogo y escaló a una persona. Menú de intenciones (`ficha.menu`, nuevo) + catálogo DERIVADO de `product` (nunca declarado aparte). Nueva acción `send_menu` (mismo contrato que `send_image`, degrada a texto si no cabe). ⚠️ El diseño original asumió "ningún negocio pasa de 10 productos" — Lis tiene 15: se agregó el mismo día el **nivel 2** (categorías → productos, con "Volver"), porque el límite de WhatsApp es 10 filas TOTALES, no por sección. 1016 pruebas en verde |
| [140-REQUISITO-FUERA-DE-CATALOGO-BLOQUEABA-EL-GUARDADO.md](140-REQUISITO-FUERA-DE-CATALOGO-BLOQUEABA-EL-GUARDADO.md) | 🔴 **Lis no podía guardar NADA en el cuestionario de alta** — un requisito "dirección" (fuera de las 4 casillas que gestiona "Datos que deben solicitarse", que solo cubren lo que se guarda en el contacto) hacía que el schema de validación rechazara el body entero con un 400, aunque nadie hubiera tocado ese campo. Verificado contra la base real: la ficha aplicada de Lis no tiene "dirección" declarada hoy — el id venía de un navegador con datos viejos, no de un dato real que hubiera que rescatar. `requisitoSchema.id` deja de ser un enum estricto y pasa a `z.string()`; nueva `fusionarRequisitosDelCatalogo()` preserva lo declarado fuera de este catálogo (si existiera en la ficha real) sin dejar que un id fuera del navegador bloquee o invente nada. 987 pruebas en verde |
| [139-CAMPOS-DE-TEXTO-AMPLIABLES.md](139-CAMPOS-DE-TEXTO-AMPLIABLES.md) | 🔑 **24 campos de texto de una línea ahora se pueden ampliar arrastrando la esquina**, como ya podía el campo "qué vendes" (un `Textarea` hereda `resize: vertical` de Tailwind; un `Input` no puede tenerlo — es una limitación del `<input>` HTML). Nuevo componente `ExpandableInput` que se ve igual a `Input` pero es un `<textarea rows={1}>` con Enter=submit (Shift+Enter=salto de línea), aplicado en las pantallas donde el dueño del negocio configura su cuenta. Los campos cortos por naturaleza (precios, horas, IDs técnicos) y los dos con atajo de Enter propio se dejaron sin tocar. 983 pruebas en verde; sin verificación visual en navegador |
| [138-PREGUNTAS-FRECUENTES-NO-SE-VUELCAN.md](138-PREGUNTAS-FRECUENTES-NO-SE-VUELCAN.md) | 🔑 **El bot de Lis, al elegir "Preguntas frecuentes" del menú, mandaba TODO su conocimiento de golpe** y le pedía a la clienta que dijera cuál quería que le respondiera — nadie preguntó primero qué quería saber. Nueva regla universal `PREGUNTAS_FRECUENTES` en `conducta.ts`: sin literal que comparar, no cabe guardarraíl (mismo límite que ya trazó el guardarraíl 7 en el [125](125-CONTENIDO-OBLIGATORIO-VERIFICADO-POR-CODIGO.md)), así que vive solo como conducta, igual que `NO_ENCAJA` y `FUERA_DE_HORARIO`. 983 pruebas en verde. ⚠️ No tiene efecto en los prompts ya guardados sin `regenerar:flota` — decisión pendiente del dueño |
| [137-REQUISITOS-COMO-PASO-DEL-ALTA.md](137-REQUISITOS-COMO-PASO-DEL-ALTA.md) | ✅ **"Qué datos pedir antes de confirmar" y "pago por adelantado de citas" ahora son parte del cuestionario de alta**, no tarjetas aparte en "Agente de IA" — se eliminaron `RequisitosSection`, `PagoCitasSection` y sus dos endpoints, un solo lugar para cada uno. `cierre.requisitos` y `cierre.pagoAntesDeLaCita` viven en la misma sección de la ficha (`flujo`) y `fusionarFicha` fusiona por sección completa — se preservan con `??` para no repetir el incidente del 15-ago. Sin prueba de integración específica ni prueba manual en navegador — pendiente antes de confiar del todo con Lashes Valen |
| [136-CATALOGO-COMPLETO-DESDE-EL-CRM.md](136-CATALOGO-COMPLETO-DESDE-EL-CRM.md) | ✅ **Crear/editar productos, grupos y opciones desde el CRM** — antes solo se podía cambiar si un grupo repite; todo lo demás se cargaba como texto libre en el alta y quedaba huérfano tras migrar. El paso "Lo que vendes" del onboarding se elimina en los dos verticales (mismo patrón que ya usaba citas → Servicios, ahora pedidos → Catálogo). Los tres negocios reales ya tenían su catálogo en tablas; no hizo falta migrar nada |
| [135-FORMAS-DE-PAGO-DESDE-LA-FICHA-CASO-NEQUI.md](135-FORMAS-DE-PAGO-DESDE-LA-FICHA-CASO-NEQUI.md) | 🔴 **El agente de Lis rechazó un pago por Nequi** ("solo aceptamos transferencia bancaria") porque las formas de pago de PEDIDOS vivían como prosa libre en `instructions`, sin dato estructurado — a diferencia de CITAS, que ya lee `ficha.pago` fresco en cada turno. Nuevo interruptor `payment_source` (Fase 1, mismo patrón que `catalog_source`) + `pagoDePedidosParaElPrompt`, que prohíbe el rechazo categórico ante un método no declarado. Encendido y verificado en vivo solo para Lis; La Churra y Lashes Valen sin cambios. No es el mismo bug que el [132](132-CONFIRMA-PAGO-SIN-VERIFICAR.md) |
| [134-AUDITORIA-AISLAMIENTO-ENTRE-CLIENTES.md](134-AUDITORIA-AISLAMIENTO-ENTRE-CLIENTES.md) | ✅ **No hay fuga de información entre La Churra, Lis y Lashes Valen**: el archivo del bot viejo de Lis no lo lee ningún proceso (archivado por higiene), el segundo bot de WhatsApp que corría en paralelo (`bot-liz-sin-ia`) ya no existe en el servidor, y el aislamiento por número está garantizado por índices únicos en `meta_credentials` — no depende de que el código filtre bien |
| [133-TURNO-MUDO-SIN-REPLY.md](133-TURNO-MUDO-SIN-REPLY.md) | 🔴 **`provide_requirement`/`update_lead` sin `reply` dejaban al cliente sin una sola palabra**: sin error, sin fila de mensaje, sin handoff — la clienta de Lis dio nombre, teléfono y dirección y el agente calló 4 minutos hasta que la dueña contestó a mano. `reply` es opcional en el esquema y el ejecutor solo envía `if (action.reply)`; ningún guardarraíl de texto podía verlo (`textosAlCliente` ya da `[]`). Chequeo 100% determinístico (revisa ausencia, no adivina texto): detecta → reintenta → deriva a una persona |
| [132-CONFIRMA-PAGO-SIN-VERIFICAR.md](132-CONFIRMA-PAGO-SIN-VERIFICAR.md) | 🔴 **El agente confirmó un pago que nunca existió**: la clienta solo avisó el MEDIO de pago ("pago por nequi") y el bot respondió "¡Recibimos tu pago con éxito!". El prompt ya lo prohibía (`prompts.ts:426`) y no bastó. Calibrado contra 82 respuestas reales de los tres negocios: el patrón correcto SIEMPRE habla de recibir el comprobante y pasarlo a verificar, nunca "recibimos tu pago" a secas — 0 falsos positivos. En el incidente real no hubo pérdida de dinero: un humano corrigió en 15 segundos |
| [131-WABA-ID-CAPTURADO-Y-USADO-EN-PLANTILLAS.md](131-WABA-ID-CAPTURADO-Y-USADO-EN-PLANTILLAS.md) | ✅ **Cierra el hueco de las plantillas para clientes con YCloud propio**: el WABA real de Meta se captura sin que nadie lo pida, en el primer mensaje que procesa el webhook (`captureMetaWabaId`, fire-and-forget), y `createTemplate`/`syncTemplates` lo usan en vez del identificador sintético `ycloud:<numero>` — con un error accionable si aún no llegó ningún mensaje. 10/10 pruebas nuevas, sin tocar base de datos |
| [130-ORQUESTADOR-FORMALIZADO-Y-GUARDARRAIL-EXTENDIDO.md](130-ORQUESTADOR-FORMALIZADO-Y-GUARDARRAIL-EXTENDIDO.md) | ✅ **El guardarraíl 7 crece a correos y teléfonos** (antes solo enlaces), con la misma regla de admisión —forma inconfundible— que evitó el falso positivo de Rappi. El refactor cosmético del orquestador se descartó a propósito: exigiría inventar un motor de control de flujo, justo lo que el doc 128 deja fuera de esta ronda. 968/968 pruebas en verde |
| [129-WABA-ID-REAL-PARA-YCLOUD-PROPIO.md](129-WABA-ID-REAL-PARA-YCLOUD-PROPIO.md) | 🔑 **Dónde vive el WABA ID real de Meta** para un cliente con cuenta propia de YCloud, separado del identificador sintético `ycloud:<numero>` que hoy ocupa `waba_id`. Columna `meta_credentials.meta_waba_id`, aditiva y nullable — no reescribe ninguna fila existente. Solo el esquema; la captura y el uso quedan en el [131](131-WABA-ID-CAPTURADO-Y-USADO-EN-PLANTILLAS.md) |
| [128-COHERENCIA-ARQUITECTURA-PROMPT-MAESTRO.md](128-COHERENCIA-ARQUITECTURA-PROMPT-MAESTRO.md) | 🔑 **Una propuesta externa de arquitectura, contrastada contra el repo real**: el Orchestrator y el Validation Engine que pedía como nuevos ya existían con otro nombre; el Flow Engine y las BehaviorRules libres quedan pospuestos por decisión ya tomada (esperar un 3er vertical); las plantillas y las campañas de remarketing eran el único hueco real. Fija el scope exacto de esta ronda (128-131) y la hoja de ruta de lo que sigue |

| [127-ESCALADA-SIN-CAUSA-ENCONTRADA.md](127-ESCALADA-SIN-CAUSA-ENCONTRADA.md) | ⬜ **El agente derivó a una persona ante un simple "Buenas tardes", sin que nada configurado se lo pidiera.** Tres hipótesis descartadas con evidencia real —no es el bug del contacto duplicado, no es un relevo humano atascado (la regla de 2 horas ya lo había liberado 4 días antes), y ninguna regla de escalado de Lis cubre ese mensaje. Probado en limpio con el pipeline real: el saludo solo, sin historial, **5/5 no escaló**. Se documenta precisamente por NO haber encontrado la causa: sin un patrón reproducible no hay literal que forzar ni tasa de fallo que medir, así que queda anotado, no arreglado — con el protocolo exacto de qué hacer si vuelve a pasar |
| [126-CONTACTO-DUPLICADO-MENSAJES-PERDIDOS.md](126-CONTACTO-DUPLICADO-MENSAJES-PERDIDOS.md) | 🔴✅ **87 webhooks fallidos en 18 días, 44 mensajes REALES de clientas perdidos sin dejar una fila.** Cuando WhatsApp identifica a un cliente por teléfono y por BSUID en momentos distintos, `getOrCreateContact` intentaba un `UPDATE` sin manejo de conflicto para rellenar la señal que faltaba — si esa señal ya era de OTRO contacto (el mismo cliente, guardado dos veces), el `UPDATE` chocaba con el índice único de la base y **el webhook entero fallaba**. Arreglado en la raíz (no con `try/catch`): `buscarPorIdentidad` ya trae todos los candidatos, así que basta con no ofrecer como "falta" nada que otro ya tenga — matemáticamente imposible que vuelva a chocar por esta vía. Los 3 pares reales de Lis (Nathalia, Tatis💙, Alejandra Ramos) se fusionaron con respaldo previo: sus conversaciones ahora leen como UNA charla continua, verificado que no se perdió ni uno de los 42 mensajes |
| [125-CONTENIDO-OBLIGATORIO-VERIFICADO-POR-CODIGO.md](125-CONTENIDO-OBLIGATORIO-VERIFICADO-POR-CODIGO.md) | ✅ **El 7º guardarraíl, y el primero que dispara con lo que dice el CLIENTE, no con lo que promete el agente.** Pedido del dueño: "que el agente lea el CRM sí o sí… para todos, no para un cliente". Genérico de verdad — sin campo nuevo, lee `kb_entry.question` y las reglasPropias con disparador condicional ("siempre que…") que ya traigan un enlace. Garantía en 3 pasos: el modelo intenta → si falta, un reintento nombrando el enlace exacto → si sigue faltando, **el servidor lo añade él mismo**, sin tercera llamada. 🔴 Casi se despliega con un falso positivo: la regla de Rappi de Lis (afirmación llana, sin "cuando…") compartía la palabra "pedido" con la del catálogo y se disparaba con CUALQUIER pedido — arreglado exigiendo un marcador condicional y acotando las palabras clave a la cláusula del disparador, nunca a la de la acción. Medido con el pipeline real: 33% → 25% → 12,5% → **0% de fallo**. ⚠️ Es código de aplicación: requiere el despliegue de tres pasos, a diferencia de los arreglos anteriores de la semana |
| [124-EL-ASESOR-ENVIO-EL-ENLACE-QUE-EL-BOT-PROMETIO.md](124-EL-ASESOR-ENVIO-EL-ENLACE-QUE-EL-BOT-PROMETIO.md) | ✅ **Arreglado y medido con el pipeline real.** El bot le dijo a Maricel «te comparto nuestro catálogo» y no mandó el enlace; un asesor lo pegó a mano 23 segundos después (`ai_generated=false` lo prueba). La causa no era el dato perdido — **el dueño YA tenía escrita la regla** «SIEMPRE que pregunte por productos envíale el link del catálogo», pero (a) el disparador no cubría «quiero hacer un pedido» y (b) la regla y el enlace vivían en dos sitios del prompt: el modelo tenía que inferir la conexión. Medido ANTES: 33% de fallo en 6 corridas reales. Tras meter el enlace DENTRO de la propia regla y ampliar el disparador: **12,5% de fallo en 8 corridas** (2,6× menos). 🔑 El último tramo no se cerró a propósito: la única falla restante no incumplió esta regla, no aplicó NINGUNA — es un rasgo probabilístico del modelo, y perseguirlo con más texto o un guardarraíl habría sido el atajo que se pidió no tomar primero |
| [123-EL-SABOR-QUE-NADIE-LEYO.md](123-EL-SABOR-QUE-NADIE-LEYO.md) | 🔴 **DIAGNOSTICADO, NO ARREGLADO — sigue vivo.** El agente inventó que el Cremoso de Temporada «hoy es de mango biche»; mango biche no existe en ningún sitio de la base. La causa: en la tabla que el agente lee, ese producto está **sin descripción** — un nombre que anuncia un atributo variable, sin valor. Y el sabor **sí estaba escrito**, en `ficha.negocio.catalogo`, el campo que el dueño edita en el cuestionario… que para un cliente en **Fase 1 está muerto**. El mismo dato en cuatro copias y el dueño escribiendo en la única que no manda. ⚠️ Con la ficha como está hoy, reejecutar `migrar:catalogo` metería el sabor **dentro del nombre** del producto. Incluye la cronología desde los respaldos (la migración NO perdió nada) y los dos bloques del prompt que se declaran ambos «la fuente de verdad» |
...
| [117-EL-CUESTIONARIO-VEIA-VACIO.md](117-EL-CUESTIONARIO-VEIA-VACIO.md) | 🔑 **El cuestionario del alta mostraba en blanco lo que el agente sí tenía**: leía un BORRADOR (`organization.metadata`) que es otra cosa que la ficha aplicada, y los tres negocios se configuraron por script — ninguno tenía borrador. No era cosmético: al enviar, el formulario **reemplaza la sección `negocio` entera**, así que partiendo de vacío se perdían en silencio los regalos, las variantes y la ubicación. Ahora se FUSIONA (ficha como base, borrador encima): elegir uno de los dos falla por los dos lados, y las dos formas se cometieron el mismo día. ⚠️ Deja al descubierto un segundo defecto SIN arreglar: el cuestionario pide `saludoInicial` y `reglasPropias` (sección `flujo`) que **no puede guardar al reeditar** — es decisión de producto |
| [116-LA-MODALIDAD-DEL-PEDIDO.md](116-LA-MODALIDAD-DEL-PEDIDO.md) | 🔑 **Quien decía "paso a recogerlo" tenía que dar dirección para cerrar**, y el modelo acababa escribiendo `"Recoge en el local"` DENTRO del campo dirección. Causa: CoreX sabía representar **lo que el negocio ofrece** y no **lo que el cliente eligió** — el estado del pedido no tenía modalidad, así que la condición del requisito solo podía preguntarle a la ficha. Se añade `modalidadDeEntrega` al estado (normalizada contra lo que la ficha ofrece, nunca inventada) y `soloEnModalidades` al requisito: las dos preguntas, cada una a su fuente. **Los cinco consumidores de requisitos no se tocaron** — solo cambia el contenido de la lista, no su forma. ⚠️ `SCHEMA_VERSION` NO sube, y se justifica: subirla haría que un contenedor sin desplegar DESCARTE pedidos en curso. Regla conservadora: si no se sabe la modalidad, el requisito se sigue pidiendo — «no se sabe» no es «no hace falta» |
| [115-LIS-POR-SECCIONES-Y-EL-CATALOGO-DUPLICADO.md](115-LIS-POR-SECCIONES-Y-EL-CATALOGO-DUPLICADO.md) | 🔑 **Convertir la ficha de Lis parecía un comando y destapó un error vivo**: `convertir:ficha` abortó porque su prompt guardado no coincidía con el recompilado — llevaba **el menú DOS veces**, uno incrustado desde la ficha y otro inyectado desde las tablas que se declara «la fuente de verdad». Causa: el 19-ago se encendió `catalog_source='tabla'` **sin regenerar el prompt**. Lección: *encender una bandera no basta, hay que recompilar lo que dependía de ella*. Al regenerar apareció además que **los tres prompts han derivado** de `conducta.ts` (La Churra +927, Lashes Valen +185): NO se tocaron, es decisión del dueño. Y `regenerar:flota` gana el filtro por cliente que ya tenían los demás scripts — «a todos» tiene que ser una elección, no el único modo |
| [114-LA-FICHA-NUEVA-NACE-POR-SECCIONES.md](114-LA-FICHA-NUEVA-NACE-POR-SECCIONES.md) | 🔑 **Todo negocio nuevo nacía con la ficha en el formato anterior al 15-ago**, y solo se convertía si alguien corría `convertir:ficha` a mano: `aplicarFicha` —el único camino que crea fichas— serializaba «como estaba», y una ficha nueva no tiene formato que conservar. Sin efecto en producción (la protección de «un dueño por dato» se deduce de los nombres de campo, no del JSON), pero **era el único hallazgo de la auditoría cuya gravedad crece con cada cliente vendido**. Se resuelve con una condición, no con un guardarraíl que avise de fichas planas: avisar de algo que se puede dejar de producir es tapar. ⚠️ La prueba que importa no es la del formato, es **«ningún campo se queda sin sección»**: `aSecciones` descarta lo que no esté en su lista, y desde este cambio un campo huérfano desaparecería al guardar |
| [113-CITAS-EN-FASE-2.md](113-CITAS-EN-FASE-2.md) | 🔑 **Los tres negocios quedan en la arquitectura nueva** — y de los dos bloqueadores anotados para citas, **uno no era un bug y el otro no era el que estaba escrito**. (1) La agenda NO estaba rota: la prueba en rojo describía la regla del cierre **anterior** al 18-ago; el único hueco que ofrece es el minuto exacto en que termina la cita previa, y empezar ahí es legítimo. Se corrigió la prueba, no el motor → 46/46. (2) Lo que SÍ fallaba: la llamada que sigue a `consult_availability` era la única del camino de citas sin esquema exigido al proveedor, y con la Fase 2 encendida tumbaba la conversación a handoff — la clienta leía "te comunico con una persona" con los horarios ya calculados. Escondido tras una bandera que nadie había encendido en citas. ⚠️ Además: **`isTest` NO impide crear citas reales**, solo bloquea el envío por WhatsApp |
| [112-PLAN-ALINEAR-LA-FLOTA.md](112-PLAN-ALINEAR-LA-FLOTA.md) | 🔑 **El plan para que los tres negocios queden con catálogo en tablas y Fase 2**: está mucho más cerca de lo que parece — los requisitos ya están declarados en las tres fichas, y lo único que falta es encender la bandera en dos. ⛔ Bloquea todo **desplegar** (con el contenedor viejo la Fase 2 de Lis está encendida pero no protege nada). ⚠️ Y lo que NO hay que hacer: poner `catalog_source='tabla'` en Lashes Valen — es un concepto exclusivo de pedidos, un salón lee `service` siempre. Orden por riesgo: desplegar → La Churra (bajo) → Lashes Valen (estrena citas en Fase 2, con dos bloqueadores propios: la agenda que ofrece huecos con el día lleno, y el esquema de `reserva` sin probar) |
| [111-LIS-EN-LA-ARQUITECTURA-NUEVA.md](111-LIS-EN-LA-ARQUITECTURA-NUEVA.md) | 🔑 **Lis pasa de prompt manual a la arquitectura completa**: ficha, catálogo en tablas y estado en el backend — la primera de la flota con las tres. El "92 contra 83" que la tenía pausada semanas **no era comparable**: medido el mismo día con el mismo juez, el prompt manual sacó 75, igual que el generado (y el manual alucinó un total). Por el camino salieron tres bugs de la plataforma, ninguno suyo: dos scripts de flota sin compilar desde el 15-ago, el lector de catálogo confundiendo la onzada con el precio (`Cremoso 7 oz` → $7) y las cabeceras con emoji leídas como productos. Todos los pasos son de DATOS: ni una línea de código específica para Lis |
| [110-EL-MODELO-NO-EMITIA-EL-ESTADO.md](110-EL-MODELO-NO-EMITIA-EL-ESTADO.md) | 🔴 **La Fase 2 nunca estuvo encendida de verdad**: el modelo NUNCA devolvía la clave `estado`, y como el esquema la tenía opcional, la respuesta pasaba a la primera — sin reintento, sin aviso, sin rastro. El pedido seguía armándolo el modelo, aparentando validación del servidor. Se cayeron cinco hipótesis con 30 llamadas reales (posición del mensaje, forma del historial, esquema opcional, contrato "ÚNICAMENTE una acción", tamaño del prompt): la causa era que **el JSON se pedía por escrito y se esperaba obediencia**. Arreglo: `response_format: json_schema` en el adaptador — 0/3 → 3/3. ⚠️ La trampa: un esquema permisivo devuelve el estado pero **la acción pierde el `text`** y el cliente se queda mudo |
