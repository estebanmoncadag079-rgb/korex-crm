...
| [102-REQUISITO-NOMBRE-EN-CITAS.md](102-REQUISITO-NOMBRE-EN-CITAS.md) | 🔴 **NADA IMPLEMENTADO — auditoría y diseño, interrumpidos a propósito.** Por qué una cita se confirma sin pedir el nombre (Fase 2 apagada → `requisitosDe()` nunca se evalúa → ningún cliente tiene requisitos declarados de todos modos). Las tres alternativas auditadas contra 7 criterios, la decisión (guardarraíl, reutilizando lo genérico) y **el bucle real que frenó el diseño**: `contact.name` no se actualiza con lo que el cliente dice en el chat. Empezar por "Lo que falta para cerrarlo" |
