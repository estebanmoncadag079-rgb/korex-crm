...
| [141-MENU-GUIADO-DE-WHATSAPP.md](141-MENU-GUIADO-DE-WHATSAPP.md) | 🔑 **El cliente TOCA una opción en vez de escribir texto que el modelo tiene que adivinar** — nace de que el bot de Lis no conectó "torta de chocolate" con "Porción Chocolate" del catálogo y escaló a una persona. Consultados diseño UX y arquitectura antes de tocar código: nada de árboles de listas encadenadas, un menú de intenciones (`ficha.menu`, nuevo) + una lista de categorías DERIVADA de `product` (nunca declarada aparte, mismo error que ya corrigió `catalog_source`). Nueva acción `send_menu` (mismo contrato que `send_image`, degrada a texto si no cabe en los límites de WhatsApp), interruptor `menu_mode` (Fase 1, apagado por defecto) y `pnpm migrar:menu`. 1008 pruebas en verde; probar en el Laboratorio y encender para Lis quedan para cuando el dueño decida |
