...
if (!url) {
  console.error("[probar-citas] DATABASE_URL no está definida");
  process.exit(1);
}
...
if (!profile) {
  console.error(`[probar-citas] No existe la organización ${organizationId}`);
  await sql.end();
  process.exit(1);
}
if (!profile.appointmentsEnabled) {
  console.error(
    "[probar-citas] Esta organización no tiene el vertical de citas encendido. Actívalo en /admin (tarjeta del cliente → Funcionalidades)."
  );
  await sql.end();
  process.exit(1);
}
if (!profile.enabled) {
  console.warn(
    "[probar-citas] Aviso: el agente está APAGADO para este cliente. El Laboratorio/sandbox igual lo prueba (evalúa el comportamiento configurado), pero recuerda encenderlo antes de ir a producción."
  );
}
...
console.log(
  `\n[probar-citas] Conversación de prueba ${conversation.id} (is_test — nunca toca WhatsApp de verdad)\n`
);
...
console.log(
  `[probar-citas] Listo. Revisa /appointments (y la bandeja) en la app con esta organización activa para ver lo que quedó guardado.`
);
await sql.end();
process.exit(0);
