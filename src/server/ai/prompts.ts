...
export const CONTRATO_DE_CONSULTA_DE_DOMICILIO = [
  '- {"action":"consultar_domicilio","zona":"la zona/dirección que dio el cliente"} — SIEMPRE que el cliente pregunte cuánto cuesta el domicilio a algún lugar, que quiera cambiar de zona, o que el pedido sea A DOMICILIO y te dé su dirección o su barrio, aunque no pregunte el precio: todo pedido a domicilio lleva el domicilio sumado al total ANTES de confirmar. Preferí volver a consultarla si pasó un rato desde la última vez, aunque ya la hayas dicho antes en la conversación — el servidor la recuerda igual, pero una cifra recién verificada es siempre mejor que una recordada.',
  '- La tabla de domicilios está organizada por BARRIOS. Si el servidor te dice que la dirección no identifica un barrio de la tabla, pídele al cliente el barrio ("Por favor, dime el barrio para ayudarte con el total con el domicilio") y con su respuesta vuelve a consultar. No des un total ni cierres el pedido sin el domicilio.',
